local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/atomiciscool/autobuilder/main/fixed/listing.lua"))()
local listing = loadstring(game:HttpGet("https://raw.githubusercontent.com/atomiciscool/autobuilder/main/fixed/imgui.lua"))()
...
function LoadBlocks(blockInfo, team)
    ...
    -- Building Part
	local SearchBlocks, FoundBlocks, SearchConnection = {}, {}, nil
	do
		local BlockAdded = function(part)
			part:WaitForChild("PPart", 1)
			if (not FindFirstChild(part, "PPart")) then
				return
			end
	...
	for blockType, table in next, blockInfo do
		...
		for i, block in next, table do
			if (i > (ownedAmount - usedAmount)) then
	...
	for i, block in next, FoundBlocks do
		if (block.Data.Size) then
			...
			ProgressUsed = ProgressUsed + 1
			UpdateProgression(99 - (ProgressAmount - ProgressUsed) / ProgressAmount * 49)
		end
	end
	do -- Paint Shit
		local PaintData = {}
...
function AutoFarm(boolean)
	...
	while AutofarmEnabled do
		...
		for i = 1, 10 do
			...
			LocalPlayer.Character.HumanoidRootPart.CFrame = stages["CaveStage" .. i].DarknessPart.CFrame
			wait((i == 1 and 4 or 2) + 0.1)
			gold:FireServer()
		end
...
do
	...
	do -- Elements
		...
		local Files = Main:AddDropdown("Files", function(file)
			SelectFile = file .. ".Build"
		end)
		...
		local Saving = Main:AddFolder("Saving")
		do
			Saving:AddTextBox("File Name",   function(name)
				FileName = name .. ".Build"
			end, {
				["clear"] = false
			})
			Saving:AddDropdown("Team",       function(team)
				if (team == "My Team") then
					team = LocalPlayer.Team
				end
				SelectedTeam = team
			end):Add("My Team"):Add("white"):Add("blue"):Add("green"):Add("red"):Add("black"):Add("yellow"):Add("magenta")
			Saving:AddButton("Save To File", function()
				SavePlot(FileName, SelectedTeam)
				UpdateList()
			end)
			Saving:Fold(true)
		end
		local BuildSettings = Main:AddFolder("Build Settings")
		do
			BuildSettings:AddSwitch("Safe Mode", function(boolean)
				SafeMode = boolean
			end)
			BuildSettings:AddDropdown("Speed",   function(boolean)
				BuildSpeed = 1
			end):Add("Dangerous"):Add("Fast"):Add("Slow"):Add("Safe")
			BuildSettings:AddTextBox("Size %",   function(int)
				BuildSize = int / 100
			end, {
				["clear"] = false
			})
		...
		local Folder = Main:AddFolder("Builder")
		do
			Folder:AddButton("List Blocks", function()
				ListBuild(SelectFile, BuildSize)
			end)
			Folder:AddButton("Preview",     function()
				if (#GetChildren(AutoBuildPreview) > 0) then
					ClearPreview()
				else
					PreviewFile(SelectFile, BuildSize)
				end
			end)
			Folder:AddButton("Load File",   function()
				LoadFile(SelectFile, #GetChildren(AutoBuildPreview) > 1 and 1 or BuildSize)
			end)
			Folder:Fold(true)
		end
	...
	do
		...
		Images:AddTextBox("URL", function(url)
			ImageUrl = url
		end, {
			["clear"] = false
		})
		local BuildSettings = Images:AddFolder("Build Settings")
		do
			BuildSettings:AddTextBox("Block Size (Studs)",  function(size)
				bSize = tonumber(size)
			end, {
				["clear"] = false
			})
			BuildSettings:AddTextBox("Image Size (Number)", function(size)
				iSize = tonumber(size)
			end, {
				["clear"] = false
			})
			local blocksDrop = BuildSettings:AddDropdown("Block Type", function(blockType)
				BlockType = blockType
			end)
			do
		...
		local Builder = Images:AddFolder("Builder")
		do
			Builder:AddButton("List Blocks", function()
				ListImage(ImageUrl, BlockType)
			end)
			Builder:AddButton("Preview",     function()
				if (#GetChildren(AutoBuildPreview) > 0) then
					ClearPreview()
				else
					BuildImage(ImageUrl, BlockType, bSize, iSize, true)
				end
			end)
			Builder:AddButton("Load Image",   function()
				BuildImage(ImageUrl, BlockType, bSize, iSize, false)
			end)
			Builder:Fold(true)
		end
	end
	...
	do
		local Mult = 1;
		local PositionOff = Adjustments:AddFolder("Position Offset")
		do
			PositionOff:AddTextBox("Move Multiplier", function(float)
				Mult = float
			end, {
				["clear"] = false
			})
			PositionOff:AddButton("Move Up",          function()
				UpdatePreview(Vector3new(0, Mult, 0) )
			end)
			PositionOff:AddButton("Move Down",        function()
				UpdatePreview(Vector3new(0, -Mult, 0))
			end)
			PositionOff:AddButton("Move Left",        function()
				UpdatePreview(Vector3new(Mult, 0, 0) )
			end)
			PositionOff:AddButton("Move Right",       function()
				UpdatePreview(Vector3new(-Mult, 0, 0))
			end)
			PositionOff:AddButton("Move Forwards",    function()
				UpdatePreview(Vector3new(0, 0, Mult) )
			end)
			PositionOff:AddButton("Move Backwards",   function()
				UpdatePreview(Vector3new(0, 0, -Mult))
			end)
			PositionOff:Fold(true)
		end
		local RotationOff = Adjustments:AddFolder("Rotation Offset")
		do
			RotationOff:AddSlider("X", function(X)
				RotationX = X
				UpdatePreview()
			end, {
				min = 0,
			...
			RotationOff:AddSlider("Y", function(Y)
				RotationY = Y
				UpdatePreview()
			end, {
				min = 0,
			...
			RotationOff:AddSlider("Z", function(Z)
				RotationZ = Z
				UpdatePreview()
			end, {
				min = 0,
		...
		local OtherOff = Adjustments:AddFolder("Other")
		do
	...
	do
		local Autofarm = Other:AddFolder("Auto Farm")
		do
